...
export class BabyProfileComponent implements OnInit {
  ...
  loadPage(page?: number, dontNavigate?: boolean): void {
    ...
    this.babyProfileService
      ...
      .subscribe({
        next: (res: HttpResponse<IBabyProfile[]>) => {
  ...
  trackId(_index: number, item: IBabyProfile): number {
    return item.id!;
  }
  ...
  delete(babyProfile: IBabyProfile): void {
    const modalRef = this.modalService.open(BabyProfileDeleteDialogComponent, { size: 'lg', backdrop: 'static' });
    modalRef.componentInstance.babyProfile = babyProfile;
  ...
  protected onSuccess(data: IBabyProfile[] | null, headers: HttpHeaders, page: number, navigate: boolean): void {
